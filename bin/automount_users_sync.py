...
"""
    This script crawls known mount points containing user home directories
    and ensures that a Mac automount entry exists for each user.

"""

MOCK = False

USERS_PATHS = ["/net/nas1.corp.jauntvr.com/users1", "/net/nas4.corp.jauntvr.com/users1", "/net/nas5.corp.jauntvr.com/users1"]

EXTRA_USERS_DIRS = ["_skel"]
EXCLUDE_USERS_DIRS = ["vpn_8aa7829d374b", "diradmin"]

LDAP_HOSTNAME="ldap.corp.jauntvr.com"
LDAP_BASEDN="dc=corp,dc=jauntvr,dc=com"

USER_MOUNT_ADD_LDIF_TEMPLATE = """
dn: cn=%(filesystem)s/%(username)s,cn=mounts,%(base_dn)s
mountType: nfs
mountDirectory: /jaunt/users/%(username)s
objectClass: mount
objectClass: top
mountOption: bg
mountOption: soft
...
mountOption: nolocks
cn: %(filesystem)s/%(username)s
"""

def generate_automount_map_for_homedir(username, server_hostname, server_export_path):
    filesystem = server_hostname + ":" + server_export_path
    result = (USER_MOUNT_ADD_LDIF_TEMPLATE % 
                  { "username" : username, 
...
def fetch_user_entities():
    return [item.pw_name for item in pwd.getpwall() 
            if item.pw_passwd=="*" and 
               item.pw_shell=='/bin/bash']   

def fetch_foxpass_users():
    API_KEY="xUAJW1FBxbTYRWZd1huW5LyTkQgN8FGX"
    headers = {"Authorization" : "Token " + API_KEY}
    response = requests.get("https://api.foxpass.com/v1/users/",
                            headers=headers)
...
def main():
    ...
    if (len(sys.argv) == 3 and "-p" in sys.argv) or len(sys.argv) == 1:
        valid_users = EXTRA_USERS_DIRS + fetch_ldap_users() + fetch_foxpass_users()
        users_paths_listings = gather_active_dir_listings(USERS_PATHS)
        desired_map_data = []
        for username in valid_users:
            if username in EXCLUDE_USERS_DIRS:
                continue
            ...
            desired_map_data += generate_automount_map_for_homedir(username, 
                                                                   hostname, 
                                                                   export_path)
